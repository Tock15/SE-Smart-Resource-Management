...
class BookingState(rx.State):
    ...
    async def submit_booking(self):
        # if self.resource["type"] == "locker" or self.resource["type"] == "equipment":
        calendar_state = await self.get_state(CalendarState)
        start = calendar_state.start_date
        end = calendar_state.end_date
        month = calendar_state.getMonth()
        ...
        if not start or not end:    
            return
        if end =="":
            calendar_state.add_disabled_day(end)
            calendar_state.reset_dates()
        else:
            calendar_state.add_disabled_range(start, end)
            calendar_state.reset_dates()
...
def equipmentandlocker_page() -> rx.Component:
    time_slots = [
        "08:00 - 09:00",
        "09:00 - 10:00",
        "10:00 - 11:00",
